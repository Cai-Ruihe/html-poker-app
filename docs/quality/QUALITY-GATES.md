...
| Airplane | Generated `file://` artifact boots with no observed external request; local Chromium runs the file-based two-way QR direct-WebRTC journey, while GitHub CI runs the identical generated artifact from a runner-local HTTP origin matching the Pages field-build delivery shape. The journeys cover local-only private reveal, wake-lock request, `pagehide` recovery-race regression, closed-phone same-seat replacement, camera controls with saved-image fallback, and synthetic-camera QR decoding. Headless Mobile WebKit boots the file but has no local ICE candidate in the current eight-second probe, so it is not counted as a pairing pass. | WAN-removed physical-device matrix, client isolation, real camera/file handling, 2–10 seats plus public display, and physical iPhone background/re-pair evidence. |
