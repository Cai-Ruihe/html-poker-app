...
| Airplane | Generated `file://` artifact boots with no observed external request; Chromium runs the local two-way QR direct-WebRTC journey. Host and joining-device scan controls request a live camera with saved-image fallback, and a synthetic Chromium camera frame is decoded into an answer QR. Headless Mobile WebKit boots the file but has no local ICE candidate in the current eight-second probe, so it is not counted as a pairing pass. | WAN-removed physical-device matrix, client isolation, real camera/file handling, 2–10 seats plus public display, and refresh/re-pair evidence. |
