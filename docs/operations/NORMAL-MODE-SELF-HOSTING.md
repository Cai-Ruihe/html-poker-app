...
2. The deployer's own card-blind Connection Service, exposed through HTTPS/WSS.

The operator token exists only on the deployer's server and in the host player's password manager. It is never a repository variable, GitHub secret, static asset, invitation, or player credential. A fork with no `NORMAL_CONNECTION_SERVICE_URL` publishes an unconfigured Normal build and does **not** fall back to the project owner's relay.

The Connection Service is not a poker engine. The active host browser remains the Trusted Host. The service mints table-scoped tickets, helps peers connect, relays encrypted envelopes when direct WebRTC is unavailable, and temporarily holds encrypted display-pairing answers. It does not receive card plaintext or card keys.
...
The app origin is an origin only: scheme plus hostname and optional port, with no repository path. The token path must be absolute. Start the service:
...
2. Under **Settings → Secrets and variables → Actions → Variables**, create `NORMAL_CONNECTION_SERVICE_URL` with your own `wss://` origin.
...
5. Push to `main` or rerun the CI workflow. The deployment configures the static artifact only with your relay URL and blocks publication if the configured public relay fails its live checks.
...
It should contain your `wss://` endpoint and no token. Open that Normal page, paste the private operator token into **Private relay host token**, and create a fresh table. Player links carry only the short-lived table-scoped ticket and invitation material.
...
| Players see **No route reached the Trusted Host** | Bring the host Safari/browser tab to the foreground and use **Reconnect to table**. Check whether the service restarted. | If the service restarted, the host must refresh the relay ticket and share newly generated invitations. Old links remain stale. |
| Quick Tunnel hostname changed | Compare the tunnel output with deployed `poker-config.js`. | Update `NORMAL_CONNECTION_SERVICE_URL`, redeploy, run the doctor, and create/share fresh invitations. |
...
- Tickets, peer registrations, and display mailboxes are in memory. A Connection Service restart invalidates old invitations and requires the host to refresh the relay ticket.
...
- This recipe does not supply TURN, durable session storage, multi-region failover, monitoring, backups, DDoS protection, or a regional/China availability claim.
